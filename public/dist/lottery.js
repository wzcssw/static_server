(function () {
  ...
  var initDom = function(dom){
    ...
    $('#dh-lottery-winner').click(function() {
  ...
  var initSelector = function() {
    ...
    window.setItemSize(itemSideSize)
    setTimeout(function() {
  ...
  window.setItemSize = setItemSize;
  var positionList = [];
  var currentTarget = [];
  var winnerProfile = [];
  ...
  var stopLottery = function(){
    ...
    ///////////
    console.log("本轮中奖者名单:",history.winner);
    var name_arr = [];
    $.each(history.winner,function(i,e){
      name_arr.push(e.name)
    });
    $.ajax({
        type: "POST",
        url: "/kill_childs",
        data: {childs_name: name_arr.join(","),test: name_arr.length},
        dataType: "json",
        success: function(data){
          // setTimeout(function() {
          //   return $('#dh-lottery-winner').addClass('is-active');
          // }, 300);
          // window.location = "/?term=" + data.next_term;
          // alert(data);
          next_url = "/?term=" + data.next_term
